...
df["gasCostInDollars"] = df.apply(
    lambda x: gas_cost_in_dollars(x["receipts_gas_used"], x["gas_price"], eth_price),
    axis=1,
)
totals = sqldf(
    """
...
block_totals = sqldf(
    "SELECT block_number, COUNT(*) as txs from df group by block_number"
)
...
def read_item(hash: str):
    transaction_data = sqldf(f"SELECT * FROM df WHERE hash = '{hash}'").to_dict(
        orient="records"
    )[0]
    ...
    txs_in_block = sqldf(
        f"select txs from block_totals where block_number = {block_number}"
    ).to_dict(orient="records")[0]["txs"]
    effective_timestamp = calculate_effective_timestamp(
        block_timestamp, txs_in_block, transaction_data["transaction_index"]
    )
