...
def get_coingecko_eth_price(historic_date: date) -> float:
    ...
    params = {"date": historic_date.strftime("%d-%m-%Y"), "localization": False}
    r = requests.get(
        "https://api.coingecko.com/api/v3/coins/ethereum/history", params=params
    )
...
def convert_csv_row_to_json(row: str) -> dict:
    # A helper for streaming data. Converting the comma separated string to a dictionary based on the expected header
    row_items = row.split(",")
...
def calculate_effective_timestamp(
    block_timestamp: datetime, txs_inblock: int, tx_index: int
):
    # Assume 12 seconds per block and each transaction in the block takes and equal length of time
